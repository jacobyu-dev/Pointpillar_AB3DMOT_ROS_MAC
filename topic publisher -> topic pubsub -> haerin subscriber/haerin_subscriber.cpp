...
{
  ROS_INFO("haerin recieve msg = %d", msg->stamp.sec);   // Prints the 'stamp.sec' message
  ROS_INFO("haerin recieve msg = %d", msg->stamp.nsec);  // Prints the 'stamp.nsec' message
  ROS_INFO("haerin recieve msg = %d", msg->data);        // Prints the 'data' message
}

int main(int argc, char **argv)                         // Node Main Function
{
  ros::init(argc, argv, "haerin_subscriber");            // Initializes Node Name
  ...
  ros::Subscriber ros_tutorial_sub = nh.subscribe("haerin_Msg", 100, msgCallback);
